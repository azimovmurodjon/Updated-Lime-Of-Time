...
async function run() {
  ...
  // Check columns
  const [cols] = await conn.execute('SHOW COLUMNS FROM business_owners');
  ...
  console.log('✓ Logo URL saved to business profile data JSON field');

  // Also update dedicated column if it exists
  if (colNames.includes('businessLogoUri')) {
    await conn.execute('UPDATE business_owners SET businessLogoUri = ? WHERE id = ?', [LOGO_URL, BUSINESS_ID]);
    console.log('✓ Also updated dedicated businessLogoUri column');
  }

  // Verify
  const [verify] = await conn.execute('SELECT data FROM business_owners WHERE id = ?', [BUSINESS_ID]);
  const verifyData = JSON.parse(verify[0].data || '{}');
  console.log('✓ Verified businessLogoUri in DB:', verifyData.businessLogoUri ? verifyData.businessLogoUri.substring(0, 60) + '...' : 'NOT SET');
