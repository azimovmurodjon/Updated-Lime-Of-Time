...
export function registerAdminRoutes(app: Express): void {
  ...
  app.get("/api/admin/clients", requireAuth, async (_req: Request, res: Response) => {
    try {
      ...
      const allClientAccounts = await dbase.select().from(clientAccounts).orderBy(sql`${clientAccounts.createdAt} DESC`);
      const allClientMessages = await dbase.select().from(clientMessages).orderBy(sql`${clientMessages.createdAt} DESC`);
      res.send(clientsPage(allCli, allBiz, allClientAccounts, allClientMessages));
    } catch (err) {
