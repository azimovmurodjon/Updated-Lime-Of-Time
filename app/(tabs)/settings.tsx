...
export default function SettingsScreen() {
  ...
  const businessNavItems = [
    ...
    },
    {
      ...
      subtitle: `${(state.noteTemplates ?? []).length} template${(state.noteTemplates ?? []).length !== 1 ? "s" : ""} saved`,
      icon: "note.text" as const,
      route: "/note-templates" as const,
      color: "#6366F1",
    },
    {
      title: "Packages & Bundles",
      subtitle: `${(state.packages ?? []).filter((p) => p.active).length} active package${(state.packages ?? []).filter((p) => p.active).length !== 1 ? "s" : ""}`,
    ...
    },
    {
      ...
      subtitle: state.settings.twilioEnabled ? "Enabled" : "Disabled",
      icon: "message.fill" as const,
      route: "/sms-automation" as const,
      color: "#00897B",
    },
    {
      title: "Export Data",
