    ...
    <ScreenContainer tabletMaxWidth={0}>
      ...
      <ScrollView ref={mainScrollRef} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100, alignSelf: "center", width: "100%", maxWidth: maxContentWidth }}>
        ...
        <View style={{ paddingHorizontal: hp, paddingTop: 4 }}>
          ...
          {/* ── Packages & Bundles preview row ── */}
          {(() => {
            const activePkgs = (state.packages ?? []).filter((p: any) => p.active);
            if (activePkgs.length === 0) return null;
            return (
              <View style={{ marginBottom: 10 }}>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 8, paddingHorizontal: hp }}>
                  <Text style={{ fontSize: 12, fontWeight: "700", color: colors.muted, textTransform: "uppercase", letterSpacing: 0.6 }}>Packages & Bundles</Text>
                  <Pressable
                    onPress={() => router.push({ pathname: "/package-browser" as any, params: calLocationFilter !== null ? { locationId: calLocationFilter } : {} })}
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: hp, gap: 10 }}>
                  {activePkgs.slice(0, 6).map((pkg: any) => {
                    const includedSvcs = (pkg.serviceIds ?? []).map((id: string) => state.services.find((s: any) => s.id === id)).filter(Boolean);
                    ...
                    return (
                      ...
                      >
                        <Text style={{ fontSize: 13, fontWeight: "700", color: colors.foreground, marginBottom: 2 }} numberOfLines={1}>{pkg.name}</Text>
                        {!!pkg.description && <Text style={{ fontSize: 11, color: colors.muted, marginBottom: 6 }} numberOfLines={2}>{pkg.description}</Text>}
                        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                          ...
                          {savings > 0 && (
                            <View style={{ backgroundColor: "#22C55E20", paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6 }}>
                              <Text style={{ fontSize: 10, color: "#22C55E", fontWeight: "700" }}>Save ${savings.toFixed(2)}</Text>
                            </View>
                          )}
                        </View>
                        {pkg.sessions && pkg.sessions > 1 ? (
                          <Text style={{ fontSize: 10, color: colors.muted, marginTop: 4 }}>🔁 {pkg.sessions} sessions</Text>
                        ) : null}
                      </Pressable>
                    );
                  })}
                  {activePkgs.length > 6 && (
                    <Pressable
                      onPress={() => router.push({ pathname: "/package-browser" as any, params: calLocationFilter !== null ? { locationId: calLocationFilter } : {} })}
                      style={({ pressed }) => ({ width: 80, alignItems: "center", justifyContent: "center", opacity: pressed ? 0.6 : 1 })}
                    >
                      <Text style={{ fontSize: 12, fontWeight: "600", color: colors.primary }}>+{activePkgs.length - 6} more</Text>
                    </Pressable>
                  )}
                </ScrollView>
              </View>
